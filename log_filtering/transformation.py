...
def generate_transformed_log_XES(
    ...
    try:
        ...
        if (original_file.endswith('.csv') or original_file.endswith('.CSV')) or (
                original_file.endswith('.xes') or original_file.endswith('.XES')):
            ...
            if (
                ...
                transformed_log_content = \
                    transformed_log_content + '''\t</trace>\n''' \
                                              ...
                                              '''\t\t<string key="concept:name" ''' \
                                              '''value="{case_id}"/>\n'''.format(
                        case_id=case_id)  # to get value next to |
                case_id = case_id + 1
        ...
        with open(transformed_file, 'w', newline='') as file2:
            ...
            for index, activity in enumerate(abstracted_patterns):
                if (
                    ...
                    if type(abstracted_timestamps[index]) == list:
                        writer.writerow(
                            [case_number, abstracted_timestamps[index][0], abstracted_timestamps[index][1], activity])
                        transformed_log_content = \
                            transformed_log_content + [[str(case_number), str(abstracted_timestamps[index][0]),
                                                        str(abstracted_timestamps[index][1]), activity]]
                    else:
                        # if no abstraction is possible then start and end timestamps of activity are same
                        writer.writerow(
                            [case_number, abstracted_timestamps[index], abstracted_timestamps[index], activity])
                        transformed_log_content = \
                            transformed_log_content + [
                                [str(case_number), str(abstracted_timestamps[index]), str(abstracted_timestamps[index]),
                                 activity]]
                else:
...
def perform_transformation(event_log,
    ...
    try:
        ...
        # defining file names
        if (event_log.endswith('.xes') or event_log.endswith('.XES')) or (
                event_log.endswith('.csv') or event_log.endswith('.CSV')):
